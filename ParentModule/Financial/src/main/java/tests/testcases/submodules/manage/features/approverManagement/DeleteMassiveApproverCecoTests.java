...
import dto.submodules.manage.approverManagement.deleteMassiveApproverCeco.DeleteMassiveApproverCecoResponseDTO;
...
import tests.testhelpers.submodules.manage.features.approverManagement.DeleteMassiveApproverCecoTestHelper;
 ...
 *
 */
public class DeleteMassiveApproverCecoTests extends ManageBaseTest {

	@BeforeClass(alwaysRun = true)
	public void beforeClass()
	{
		featureTest=subModuleTest.createNode("DeleteMassiveApproverCecoTests");
	}

	/**
	 * Check that is feasible to delete an approver from multiples cecos.
	 * @param parameters
	 * @throws Exception
	 */
	...
	public void deleteAnApproverFromMultiplesCecos(MassiveApproverCecoParameters parameters) throws Exception {
		...
		response = new DeleteMassiveApproverCecoTestHelper(parameters.getUsername()).deleteMassiveApproverCeco(payload);
		validateResponseToContinueTest(response, 200, "delete an approver to multiples cecos api call was not successful.", true);
		DeleteMassiveApproverCecoResponseDTO approver = response.as(DeleteMassiveApproverCecoResponseDTO.class, ObjectMapperType.GSON);
		...
		softAssert.assertEquals(approver.getContent().getCecoFailed(), "false", "getCecoFailed issue");
		softAssert.assertAll();
	}

	/**
	 * Check that is feasible to delete an approver from all cecos.
	 * @param parameters
	 * @param isEntireCecos
	 * @throws Exception
	 */
	@Test(dataProvider = "massiveApproverCeco", dataProviderClass = ApproverManagementDataProviders.class, 
			groups = { ExeGroups.NotAvailableInPreProd })
	public void deleteAnApproverToAllCecos(MassiveApproverCecoParameters parameters) throws Exception {
		SoftAssert softAssert = new SoftAssert();
